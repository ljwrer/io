...
var create = function create(defaultConfig) {
  ...
  ['get', 'delete', 'head', 'options', 'post', 'put', 'patch'].forEach(function (method) {
    io[method] = function (url, requestData, success, fail, config) {
      ...
      return new Promise(function (resolve, reject) {
        fetch.request(configs.config).then(function (res) {
  ...
  io.jsonp = function (url, requestData, success, fail, config) {
    var configs = normalizeArgs('jsonp', url, requestData, success, fail, config);
    configs.config = _.merge({}, defaults, defaultConfig, configs.config);
    configs.url = buildURL(combineURLs(configs.config.baseURL, configs.config.url), configs.config.params);
    return new Promise(function (resolve, reject) {
      jsonp(configs.url, configs.config, function (err, data) {
        if (err) {
          var ret = {
            code: 500,
            message: err.message,
